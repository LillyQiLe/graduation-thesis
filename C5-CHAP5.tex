...
图\ref{fig:tl_unreduce}表示在不实施降低道路饱和度信号控制算法的情况下，由于应急车到达路口时的前面排队车辆较多，因此必须等到前面的排队车辆全部清空后方可通过路口。图\ref{fig:tl_unreduce_passed}的情况下，应急车辆通过交叉口后，前方有普通车辆阻碍了应急车辆的前进，使得应急车辆无法以更快的速度行驶。
