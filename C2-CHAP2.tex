...
%给定一个数据集合，其中包含了${n}$个样本，每个样本都是${d}$维数据点，数据集${X=\{x_1,x_2,\ldots,x_n\}}$，其中${x_i \in R^d}$，需要将数据集划分为${k}$个簇，有${C=c_1, c_2, \ldots, c_k}$，每个簇都有对应的质心，表示簇${c_i}$均值向量，质心的值为簇内样本均值，质心定义为
%\begin{equation}
%	\mu_i=\frac{1}{\left|c_i\right|}\sum_{x\in c_i} x
%\end{equation}
...
K均值聚类算法流程如下：
\begin{enumerate}
	\item 在确定了需要生成的样本集个数${k}$后，在数据集中，随机地选择${k}$个样本作为初始质心： ${{\mu_1,\mu_2,\ldots,\mu_k}}$。%由于K均值聚类算法是启发式方法，${k}$个初始化的质心的位置选择对最后的聚类结果和运行时间都有很大的影响，因此需要选择合适的质心，最好这些质心不能太近。
...
%若给定足够的迭代次数，K均值聚类算法就能收敛，但是有可能在局部最小值点收敛。K均值聚类收敛局部极值的原因很可能是初始化簇类中心的距离很接近，而且算法的收敛时间也加长了，为了避免这一情况，多次运行K均值聚类算法，每次运行初始化不同的簇类中心。

%为了避免K均值聚类算法在局部最小值点收敛，应多次运算K均值聚类算法，并且每次初始化的质心不同。
...
%${E}$代表对应等式约束集和，有${i \in E}$，${I}$代表不等式约束的集和，有${j \in I}$，假设有${m}$个等式约束集和和${n}$个不等式约束集和。${g}$、${x}$、${a_i(i \in E)}$和${h_j(j \in I)}$是${n}$维向量。
